 ...
 */
int
get_lines_needed ( const char *text, SDL_Rect t_rect,
                   float text_stretch )
{
    t_rect.h = 32767;           // arbitrary large number
    display_char_disabled = TRUE;
    int lines =
        DisplayText ( text, t_rect.x, t_rect.y, &t_rect, text_stretch );
...
display_char ( unsigned char c )
{
    ...
    if ( ( !display_char_disabled )
         && ( !display_char_disabled_local ) )
        PutCharFont ( Screen, GetCurrentFont (  ), MyCursorX, MyCursorY,
                      c );

    MyCursorX +=
        CharWidth ( GetCurrentFont (  ),
                    c ) + get_letter_spacing ( GetCurrentFont (  ) );
}
 ...
 */
void
show_backgrounded_label_at_pixel_position ( char *LabelText, int pos_x,
                                            int pos_y )
{
 ...
 */
void
show_backgrounded_label_at_map_position ( char *LabelText,
                                          float fill_status,
                                          ...
                                          int zoom_is_on )
{
    ...
    float zoom_factor =
        ( zoom_is_on ? lvledit_zoomfact_inv (  ) : 1.0 );

    translate_map_point_to_screen_pixel ( pos_x, pos_y, &pixel_x,
                                          &pixel_y, zoom_factor );
    show_backgrounded_label_at_pixel_position ( LabelText, pixel_x,
                                                pixel_y );
 ...
 */
int
show_backgrounded_text_rectangle ( const char *text,
                                   struct BFont_Info *font, int x,
                                   int y, int w, int h )
{
    ...
    t_rect.w = w;
    t_rect.h = r_height;
    our_SDL_fill_rect_wrapper ( Screen, &t_rect,
                                SDL_MapRGB ( Screen->format, 0, 0,
                                             0 ) );
...
alert_window ( const char *text )
{
    ...
    while ( 1 )
    {
        ...
        int r_height =
            show_backgrounded_text_rectangle ( text, FPS_Display_BFont,
                                               x, y, w, h );

        show_backgrounded_text_rectangle ( _( "Click to continue..." ),
                                           Red_BFont, x, y + r_height,
                                           w, 10 );
  ...
  wait_click_and_out:
    while ( SpacePressed (  ) || EnterPressed (  ) || EscapePressed (  )
            || MouseLeftPressed (  ) );
    ...
    if ( ThisRobot->faction == FACTION_BOTS )
        ...
        case 0:
            ThisRobot->TextToBeDisplayed =
                _( "Unhandled exception fault.  Press ok to reboot." );
            break;
        case 1:
            ThisRobot->TextToBeDisplayed =
                _( "System fault. Please buy a newer version." );
            break;
        case 2:
            ThisRobot->TextToBeDisplayed =
                _( "System error. Might be a virus." );
            break;
        case 3:
            ThisRobot->TextToBeDisplayed =
                _( "System error. Please buy an upgrade from MS." );
            break;
        case 4:
            ThisRobot->TextToBeDisplayed =
                _( "System error. Press any key to reboot." );
            break;
        }
    else
        ThisRobot->TextToBeDisplayed =
            _( "Aargh, I got hit.  Ugh, I got a bad feeling..." );
...
ScrollText ( char *Text, int background_code )
{
    ...
    SDL_Rect ScrollRect =
        { User_Rect.x + 10, User_Rect.y, User_Rect.w - 20,
        User_Rect.h
    };
    ...
    while ( !done )
    {
        ...
        Number_Of_Line_Feeds =
            DisplayText ( Text, ScrollRect.x, InsertLine, &ScrollRect,
                          TEXT_STRETCH );
            ...
            if ( event.type == SDL_MOUSEBUTTONDOWN )
            {
                ...
                case SDL_BUTTON_LEFT:
                    if ( MouseCursorIsOnButton
                         ( SCROLL_TEXT_UP_BUTTON, GetMousePos_x (  ),
                           GetMousePos_y (  ) ) )
                    {
                        event.key.keysym.sym = SDLK_UP;
                    }
        ...
        if ( InsertLine +
             ( Number_Of_Line_Feeds +
               1 ) * ( int ) ( FontHeight ( GetCurrentFont (  ) ) *
                               TEXT_STRETCH ) < ScrollRect.y
             && ( speed > 0 ) )
        {
            InsertLine =
                ScrollRect.y - ( Number_Of_Line_Feeds +
                                 1 ) *
                ( int ) ( FontHeight ( GetCurrentFont (  ) ) *
                          TEXT_STRETCH );
            speed = 0;
        }
...
SetNewBigScreenMessage ( const char *ScreenMessageText )
{
    ...
    while ( i > 0 )
    {
        ...
        Me.BigScreenMessageDuration[i] =
            Me.BigScreenMessageDuration[i - 1];
        i--;
    }

    Me.BigScreenMessage[0] =
        MyMalloc ( strlen ( ScreenMessageText ) + 1 );
...
DisplayBigScreenMessage ( void )
{
    ...
    {
        if ( Me.BigScreenMessageDuration[i] <
             GameConfig.delay_for_big_screen_messages )
        {
            SDL_SetClipRect ( Screen, NULL );
            CenteredPutStringFont ( Screen, Menu_BFont,
                                    next_screen_message_position,
 ...
 *-----------------------------------------------------------------*/
int
DisplayText ( const char *Text, int startx, int starty,
              const SDL_Rect * clip, float text_stretch )
{
    ...
    int tab_width =
        TABWIDTH * ( CharWidth ( GetCurrentFont (  ), TABCHAR ) +
                     letter_spacing );
                         ...
                         float text_stretch )
{
    ...
    if ( *Resttext == ' ' )
        NeededSpace =
            CharWidth ( GetCurrentFont (  ), ' ' ) + letter_spacing;
    else if ( *Resttext == '\t' )
        NeededSpace =
            TABWIDTH * ( CharWidth ( GetCurrentFont (  ), TABCHAR ) +
                         letter_spacing );
 ...
 * ----------------------------------------------------------------- */
char *
GetString ( int MaxLen, int background_code,
            const char *text_for_overhead_promt )
{
    ...
    {
        store =
            SDL_CreateRGBSurface ( 0, GameConfig.screen_width, height,
                                   vid_bpp, 0, 0, 0, 0 );
        Set_Rect ( store_rect, x0, y0, GameConfig.screen_width,
                   height );
        our_SDL_blit_surface_wrapper ( Screen, &store_rect, store,
                                       NULL );
    }
    ...
    while ( !finished )
    {
        ...
        else
        {
            ...
            our_SDL_blit_surface_wrapper ( store, NULL, Screen,
                                           &tmp_rect );
        }
        ...
        else if ( ( key < SDLK_DELETE ) && isprint ( key )
                  && ( curpos < MaxLen ) )
        {
        ...
        else if ( ( key <= SDLK_KP9 ) && ( key >= SDLK_KP0 )
                  && ( curpos < MaxLen ) )
        {
            key -= SDLK_KP0;
            key += '0';
...
#define EDIT_WINDOW_TEXT_OFFSET 15
    ...
    //
    DisplayText ( PopupWindowTitle, TargetRect.x, TargetRect.y,
                  &TargetRect, TEXT_STRETCH );
    x0 = MyCursorX;
    y0 = MyCursorY;
    ...
    while ( !finished )
    {
        ...
        DisplayText ( input, TargetRect.x, TargetRect.y, &TargetRect,
                      TEXT_STRETCH );
